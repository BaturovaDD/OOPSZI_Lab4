...
{
	return Complex(_CMATH_::exp(z.Re()), _CMATH_::exp(z.Im()));
}

Complex Complex::sh(const Complex &z)
{
	return ( exp(z) - exp( Complex(z) * Complex(-1, 0) ) ) / Complex(2, 0);
}

Complex Complex::sin(const Complex &z)
{
	return Complex(_CMATH_::sin(z.Re()) * sh(z.Im()).Re(), _CMATH_::cos(z.Re()) * sh(z.Im()).Re());
}
